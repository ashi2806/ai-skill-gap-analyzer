...
# Custom Header
render_header("AI Skill Gap Analyzer", "Architecting your career path with precision AI analysis and high-fidelity roadmaps.")
...
#             max_tokens=10
#         )
#         st.success(f"✅ API works! Response: {test_response.choices[0].message.content}")
#     except Exception as e:
...
if uploaded_file is not None and selected_role != "Select a role":
    ...
    with tab1:
        ...
        match_percent = (len(matched_skills) / total_skills * 100) if total_skills > 0 else 0
        
        st.markdown(f"""
        <div class="glass-card animate-in" style="padding: 2rem; border-top: 4px solid var(--accent-indigo); text-align: center; margin-bottom: 2rem;">
            <p style="margin: 0; font-size: 1rem; color: var(--text-dim); text-transform: uppercase; letter-spacing: 0.2em; font-weight: 700;">Target Role Percent</p>
            <h2 style="font-size: 4rem; margin: 10px 0; background: linear-gradient(135deg, #fff 30%, var(--accent-indigo) 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">{match_percent:.1f}%</h2>
            <div style="width: 100%; height: 8px; background: rgba(255,255,255,0.05); border-radius: 4px; overflow: hidden; margin-top: 20px;">
                <div style="width: {match_percent}%; height: 100%; background: linear-gradient(90deg, var(--accent-indigo), var(--accent-cyan)); box-shadow: 0 0 20px var(--accent-indigo);"></div>
            </div>
            <p style="margin-top: 15px; color: var(--text-dim); font-size: 0.95rem;">
                Match Accuracy: {len(matched_skills)} of {total_skills} critical skills aligned
            </p>
        </div>
        """, unsafe_allow_html=True)
